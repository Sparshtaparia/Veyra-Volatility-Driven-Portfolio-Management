...
export function ForgotPasswordPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); if (!email.includes("@")) { setError("Enter the email address linked to your account."); return } setError(""); try { await resetPassword(email); setSent(true) } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to send reset instructions.") } }
