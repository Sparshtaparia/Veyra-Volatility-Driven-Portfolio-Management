...
def test_get_portfolio_features_invalid_portfolio(client: TestClient):
    response = client.get(
        "/api/v1/portfolios/fake_port/features?ticker=AAPL&start_date=2023-01-01&end_date=2023-12-31"
    )
...
def test_get_portfolio_features_invalid_ticker(client: TestClient):
    ...
    response = client.get(
        f"/api/v1/portfolios/{portfolio_id}/features?ticker=AAPL&start_date=2023-01-01&end_date=2023-12-31"
    )
    assert response.status_code == 404
